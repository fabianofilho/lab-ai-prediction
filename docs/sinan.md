...
*   **Exemplo (Predição de Abandono de Tratamento de Tuberculose)**:
    ...
    *   **Outcome**: O campo `SITUA_ENCE` na ficha de encerramento do caso foi preenchido como abandono (2) ou abandono primário (10)? (Sim/Não). Cura (1) e falência (9) são Não. Óbito (3 e 4, risco competitivo), transferência (5), mudança de diagnóstico (6), TB-DR (7) e mudança de esquema (8) são censura: o caso sai da coorte em vez de virar Não.
