...
- **Procedência do benchmark.** O commit `b79bc78` do fabianofilho/datasus-ai-prediction (não do fork da org, que parou no commit anterior) continua acessível no repositório arquivado e segue sendo a procedência do código vendorizado no datasus-preprocessing-benchmark. Essa referência não muda de repositório nem de SHA.
