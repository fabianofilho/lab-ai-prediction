...
def optimize_hyperparams(
    ...
    hiperparâmetros. Em holdout e corte temporal, passe só a partição de
    treino (ver split_train_test), nunca a coorte inteira.
    """
    try:
        import optuna
...
def split_train_test(
    ...
    strategy:
        "holdout"  : train_test_split estratificado com `holdout_size`.
        "temporal" : treino = `dates` < `cutoff`, teste = `dates` >= `cutoff`
                     (`dates` alinhado por posição com X).

    Devolve X_tr, X_te, y_tr, y_te. A HPO deve receber só X_tr e y_tr.
    """
    if strategy == "temporal":
        d = pd.to_datetime(pd.Series(dates), errors="coerce")
        cut = pd.Timestamp(cutoff)
        train_mask = (d < cut).to_numpy()
        test_mask = (d >= cut).to_numpy()
    ...
    raise ValueError(f"strategy desconhecida: {strategy!r} (use 'holdout' ou 'temporal')")
